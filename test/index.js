...
async function main() {
  ...
  console.log(`Starting tests on task #1 ("${task.id()}")`);
  ...
  console.log(`Starting tests on task #2 ("${task2.id()}")`);
  ...
  const task3 = Hyper.task({ id: "test", delay: 30 });

  console.log(`Starting tests on task #3 ("${task3.id()}")`);

  if (task3.id().length !== 36) {
    throw new Error("Task ID should be 36 char");
  }
}
